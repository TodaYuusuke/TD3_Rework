...
/// パーティクル
/// </summary>
class SoilParticle : public LWP::Object::Particle<LWP::Resource::RigidModel> {
private: // ** 純粋仮想関数のオーバーライド ** //
	...
	void Generate(Data& newData) override {
		...
		// 発射のベクトル
		LWP::Math::Vector3 dir{ dir1 / 100.0f + -destinate_->x,dir2 / 100.0f + -destinate_->y, dir3 / 100.0f + -destinate_->z };
	...
	/// <summary>
	/// 自機の抜刀攻撃の方向ベクトルのアドレスを設定
	/// </summary>
	/// <param name="destinate"></param>
	void SetDestinate(LWP::Math::Vector3* destinate) { destinate_ = destinate; }
